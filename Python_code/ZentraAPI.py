...
def zentra_json_parser(json_obj, logger_port):
    ...
    for row in json_obj['device']['timeseries'][0]['configuration']['values'][0:3]:
        ...
        try:
            water_level_data = filter(lambda x: x[0]['description']=='Water Level', row[logger_port:])[0] #maybe this works?
            #water_level_data = filter(lambda x: x[0]['description']=='Water Level', row[5:])[0]
